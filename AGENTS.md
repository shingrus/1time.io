...
- **View counter (text secrets only):** `saveSecret` accepts an optional `views` field — `1` (default, burn after reading) or `2..100`. `maxViews` (100) caps it, which also bounds how many times one stored ciphertext can be retrieved (no unbounded-download amplification). Multi-view reads decrement `StoredMessage.Views` in place inside the WATCH transaction (TTL preserved via `PTTL`; retried on `TxFailedErr`); the last view deletes the record as before. `/api/get` returns `viewsLeft` (`0` = consumed) so the reader page renders honest post-read copy, and the create-page "link ready" copy reflects the chosen view count. Legacy records without the field behave as single-view. Files remain strictly one-time.
