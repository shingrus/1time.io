...
- **Status ("My Secrets"):** `POST /api/secretStatus` reports, for a batch of ids, whether each secret still exists — **without consuming it**. "Gone" means read or expired. Deliberately ungated and `EXISTS`-based, so no ciphertext leaves Redis on the polling timer. The only thing gating would buy is hiding read timing from someone holding an id but not the fragment key — and anyone with the actual link can simply read the secret, which tells them strictly more.
...
- Frontend file size limit is `Constants.maxFileSizeBytes = 99 * 1024 * 1024` in `frontend/src/lib/util.js`; keep it aligned with the backend's `maxFileSize`. Both describe the **plaintext** file; the wire limit is `maxFileUploadBodyBytes` (100 MB), which allows for the AES-GCM IV/tag and multipart overhead and matches nginx's `100m`.
...
- nginx upload ceiling is `100m` in both `configs/nginx/1time.conf` and `docker/nginx/default.conf.template` to stay above the backend's `99 MB` multipart limit, and at Cloudflare Free's own 100 MB edge cap. Upload/download timeouts on `/api/saveFile` and `/api/getFile` are `10m`; at 99 MB that needs roughly 1.4 Mbit/s sustained, so a very slow mobile uplink can time out.
