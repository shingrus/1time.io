...
- **View counter (text secrets only):** `saveSecret` accepts an optional `views` field — `1` (default, burn after reading), `2..100`, or `-1` for unlimited views until the TTL expires. Multi-view reads decrement `StoredMessage.Views` in place inside the WATCH transaction (TTL preserved via `PTTL`; retried on `TxFailedErr`); the last view deletes the record as before. `/api/get` returns `viewsLeft` (`0` = consumed, `-1` = unlimited) so the reader page renders honest post-read copy. Legacy records without the field behave as single-view. Files remain strictly one-time.
