...
- **Create:** the client generates a 20-char `randomKey` (`getRandomString`). With an optional passphrase, `fullSecretKey = passphrase + randomKey`. HKDF-SHA256 (salt `onetimelink:v2`) derives (a) an AES-256-GCM key (`info=encrypt`) and (b) `readToken` (`info=auth`, hex) — the **only** key material ever sent to the server, and only when *reading*.
- The client AES-256-GCM-encrypts the secret/file (12-byte IV prepended) and POSTs `{ciphertext, readTokenHash, v, duration}`, where `readTokenHash = SHA-256(readToken)` over the token's **hex string**. The server stores `{ciphertext, readTokenHash, v:3}` under a server-generated id — never the key, the plaintext, or the read token itself.
- **Save scheme v2 vs v3.** Records written before v3 store `readToken` directly and carry no `v` field; zero means v2, so no data migration was needed. v3 stores only the hash, which means neither a database dump nor an intercepted save request can read or destroy a secret — reading requires the preimage, which only a holder of the link can derive. See `tokenMatchesStored` in `backend/storage.go` and `hashReadToken` in `protocol.mjs`; the two must hash the same representation or every v3 secret becomes unreadable, which is why `TestInteropVectorFromProtocolMjs` pins a shared vector.
- **Link:** `/v/#<randomKey><id>` (text) or `/f/#<randomKey><id>` (file). `randomKey` lives **only** in the URL fragment and is never sent to the server.
- **Read (one-time by default):** the recipient's browser re-derives `readToken` from the fragment and POSTs `{id, hashedKey}` — **byte-identical in v2 and v3**, which is why a client that predates v3 can still open a v3 link and no client-side read migration was required. The server hashes the presented token for v3 records, compares directly for v2, constant-time in both cases and reserves one allowed read/download. The browser decrypts locally. Text ciphertext remains in Redis while views remain. File metadata remains in Redis and the encrypted disk blob remains on disk while downloads remain.
...
- **Manage token:** every save on a push-enabled deployment mints a random 32-byte `manageToken`, returns it **only in the save response** (never in the share link), and stores just its SHA-256 as `manageHash` on the record. It authorises one thing: subscribing to read notifications via `/api/subscribeToUpdates`. Compared constant-time; an empty stored hash never matches, so records predating it — and every record where push is unconfigured — cannot be subscribed.
- **Status ("My Secrets"):** `POST /api/secretStatus` reports, for a batch of ids, whether each secret still exists — **without consuming it**. "Gone" means read or expired. Deliberately **not** gated on the manage token: verifying one means `GET`ting each record, dragging whole ciphertexts out of Redis to read a hash, on a timer, for up to 128 secrets. `EXISTS` returns an integer. The only thing gating would buy is hiding read timing from someone holding an id but not the fragment key — and anyone with the actual link can simply read the secret, which tells them strictly more.
- **Read notifications (Web Push):** the creator may subscribe a browser notification for when their secret is read. `POST /api/subscribeToUpdates` attaches a push subscription, gated on the manage token; the read paths fire it asynchronously. See "Push Notifications".
...
- `/api/subscribeToUpdates` (`apiSubscribeToUpdates` in `backend/push.go`) stores a push subscription under `pushKey<id>` with the secret's own remaining TTL. Body carries two values only: `200 {"status":"ok"}` subscribed, `200 {"status":"gone"}` already read. Every failure rides the HTTP status, so a client treats any non-200 as not-subscribed without parsing a body.
...
- Zero-knowledge like the web app: encryption happens in the extension's service worker; only ciphertext and `readTokenHash` leave the browser on save; the read token itself leaves only when reading.
...
- **What it does:** on the link-ready screen only, the creator can subscribe a browser notification for when their secret is read. Nothing appears on the main page — no markup, no CSS, no JS. `showLinkReady` reaches the feature through a single dynamic `import()` and nothing else — the control itself is built by `mountNotifyControl` in `frontend/src/lib/pushNotifications.js`, which **replaces** the "Check if it's been read" link where push is available; where it is not, that link stays. Keeping it out of `show-link-ready.ts` matters: that chunk is preloaded on every page carrying a share form.
- **One service worker registration per secret**, at scope `/push/<id>/` (`frontend/src/lib/pushNotifications.js`). A single worker at `/` would reuse one push endpoint for every secret a browser creates, handing the server a durable identifier linking a sender's secrets together. Per-secret scopes cost one conditional request for the worker script per subscribe; that is the price of the unlinkability.
- **Three frontend modules, split by what each page needs.** `pushNotifications.js` holds subscribing and the link-ready control; `pushRegistrations.js` holds only registration bookkeeping (`scopeFor`, `unsubscribeFromUpdates`, `sweepStaleRegistrations`) so the My Secrets off switch can import that alone rather than 4 KB of subscribing code; `mySecrets.js` holds the per-browser opt-out. **None of the three imports `util.js`** — util statically imports `protocol.mjs`, so any page importing them would otherwise load the whole AES/HKDF bundle. `pushNotifications.js` posts to the API with its own small `fetch` for exactly that reason.
- **The worker** is `frontend/src/sw/push-sw.js`, bundled to `public/push-sw.js` by `scripts/build-push-sw.mjs` (npm `predev`/`prebuild`). The generated file is gitignored — edit the source. Bundled rather than copied so `nameForId` has one definition: `public/` bypasses Vite, so the worker cannot import at runtime. Tests: `frontend/test/push-sw.test.mjs` via `npm test` (`node --test`).
- **Nothing identifying crosses the wire.** The payload is `{id, kind, viewsLeft, readAt}`, encrypted per RFC 8291; the worker derives "brave-otter" locally with `nameForId`. There is deliberately **no `Topic` header** — headers are outside the encryption, and the only usable collapse key is the storage id, which would tell the push provider which secret each notification belongs to and let it tie that to the browser endpoint. The worker's notification `tag` collapses repeats on arrival instead.
- `readAt` is a Unix timestamp rendered as elapsed time. Push services are store-and-forward, so a notification can arrive hours after the read and must not imply "just now".
- **Copy follows the app's own vocabulary:** "viewed" for a message, "downloaded" for a file, matching what `showLinkReady` said at creation. "Opened" is avoided — on a notification it reads as the notification having been opened.
- **VAPID** keys come from `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY` and `VAPID_SUBJECT`; all three are required or push is off, and `logPushConfiguration` says which are missing at startup. The **public key rides on the save response**, not baked into the bundle, so self-hosting needs no frontend rebuild — and its absence is how a client learns push is unavailable. Generate once and never rotate casually: subscriptions are bound to the key they were created with.
...
- `/push-sw.js` is served from the site root with nginx's default static handling (ETag + Last-Modified, no explicit `Cache-Control`), so repeat registrations revalidate to `304`. It must stay at the root: a worker can only claim a scope at or below its own path, and `/push/<id>/` is below `/`.
- Push adds one endpoint to the planned Cloudflare `/api/*` allowlist — `subscribeToUpdates`, taking it from 7 to 8 — and needs outbound 443 to `fcm.googleapis.com`, `updates.push.services.mozilla.com` and `web.push.apple.com`.
