...
- **Hard constraints:** frontend Lighthouse performance is not optional — keep CSS/JS off the initial render path per the "Frontend CSS/JS Performance" sections in AGENTS.md. Keep the frontend and backend `99 MB` file limits aligned (and nginx `client_max_body_size` above them, currently `100m`, which is also Cloudflare Free's edge cap).
