...
export default function ShowNewLink({newLink = "", onReset}) {
    ...
    const [bookmarkShortcut] = useState(() => {
        if (typeof window === 'undefined') {
            return 'Ctrl+D';
        }

        const platform = window.navigator?.platform || '';
        return /Mac|iPhone|iPad|iPod/.test(platform) ? '⌘D' : 'Ctrl+D';
    });
            ...
            {newLink && (
                <>
                    ...
                    <div className="callout callout-tip" role="note">
                        ...
                        <span>Bookmark 1time.io so you can find it next time — {bookmarkShortcut}</span>
                    </div>
                </>
            )}
        </div>
