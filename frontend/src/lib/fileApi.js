...
function attemptUpload(encryptedBlob, hashedKey, durationDays, onProgress, options = {}) {
    const {pushSub = null} = options;
    ...
    if (pushSub) {
        formData.append('push_sub', JSON.stringify(pushSub));
    }
 ...
 */
export async function saveFile(encryptedBlob, hashedKey, durationDays, onProgress, options = {}) {
    let lastError;
    for (let attempt = 0; attempt < UPLOAD_MAX_ATTEMPTS; attempt += 1) {
        try {
            return await attemptUpload(encryptedBlob, hashedKey, durationDays, onProgress, options);
        } catch (error) {
            lastError = error;
